...
namespace GcjUiCtrl.Control
{
    ...
    public partial class CustomizedLabel : System.Windows.Controls.Label
    {
        public CustomizedLabel() : base()
        {
            //DefaultStyleKey = typeof(CustomizedLabel);
            Loaded += CustomizedLabel_Loaded;
            DragEnter += CustomizedLabel_DragEnter;
            DragOver += CustomizedLabel_DragOver;
            Drop += CustomizedLabel_Drop;
        }

        private void CustomizedLabel_Loaded(object sender, RoutedEventArgs e)
        {
            Debug.WriteLine($"{string.Format("{0:d5}", ++GcjConst.nDbgSequence)}: private void CustomizedLabel_Loaded Triggered(" + sender?.GetType()?.Name + " <" + (sender as FrameworkElement)?.Name + ">)");
            //Debug.WriteLine("$$$$ CustomizedLabel_Loaded::(" + sender?.GetType()?.Name + ":" + (sender as FrameworkElement)?.Name + ", " + e.OriginalSource?.GetType()?.Name + ":" + (e.OriginalSource as FrameworkElement)?.Name + ") $$$$");
        ...
        public static readonly DependencyProperty _GcjVer = DependencyProperty.Register(
            "GcjVer",
            typeof(string),
            typeof(CustomizedLabel),
            new PropertyMetadata("Gcj Label v1.0"));


        [Description("Gets or sets the CustomizedLabel Version")]
        [Category("GcjControl")]
        ...
        [Description("Gets or sets the CustomizedLabel Drop File Path")]
        [Category("GcjControl")]
        ...
        private void CustomizedLabel_DragOver(object sender, DragEventArgs e)
        {
        ...
        private void CustomizedLabel_DragEnter(object sender, DragEventArgs e)
        {
            if (this.AllowDrop == true && e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                // Allow copy action
        ...
        private void CustomizedLabel_Drop(object sender, DragEventArgs e)
        {
            if (this.AllowDrop == true && e.Data.GetDataPresent(DataFormats.FileDrop))
            {
        ...
        public static readonly RoutedEvent OnFileDroppedEvent = EventManager.RegisterRoutedEvent(
            ...
            typeof(CustomizedLabel)); // 注册事件的类

        // CLR 包装器，允许通过 += 和 -= 订阅和解除订阅事件
        [Description("Customized Label Control Drop File Event Handler")]
        [Category("GcjControl")]
