...
namespace GcjUiCtrl.Control
{
    ...
    public partial class CustomizedComboBox : System.Windows.Controls.ComboBox
    {
        public CustomizedComboBox() : base()
        {
            //DefaultStyleKey = typeof(CustomizedComboBox);
            Loaded += CustomizedComboBox_Loaded;
            DragOver += CustomizedComboBox_DragOver;
            DragEnter += CustomizedComboBox_DragEnter;
            Drop += CustomizedComboBox_Drop;
        }

        private void CustomizedComboBox_Loaded(object sender, RoutedEventArgs e)
        {
            Debug.WriteLine($"{string.Format("{0:d5}", ++GcjConst.nDbgSequence)}: private void CustomizedComboBox_Loaded Triggered(" + sender?.GetType()?.Name + " <" + (sender as FrameworkElement)?.Name + ">)");
            //Debug.WriteLine("$$$$ CustomizedComboBox_Loaded::(" + sender?.GetType()?.Name + ":" + (sender as FrameworkElement)?.Name + ", " + e.OriginalSource?.GetType()?.Name + ":" + (e.OriginalSource as FrameworkElement)?.Name + ") $$$$");
        ...
        public static readonly DependencyProperty _GcjVer = DependencyProperty.Register(
            "GcjVer",
            typeof(string),
            typeof(CustomizedComboBox),
            new PropertyMetadata("Gcj CheckBox v1.0"));


        [Description("Gets or sets the CustomizedComboBox Version")]
        [Category("GcjControl")]
        ...
        [Description("Gets or sets the CustomizedComboBox Drop File Path")]
        [Category("GcjControl")]
        ...
        private void CustomizedComboBox_DragOver(object sender, DragEventArgs e)
        {
        ...
        private void CustomizedComboBox_DragEnter(object sender, DragEventArgs e)
        {
            if (this.AllowDrop == true && e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                // Allow copy action
        ...
        private void CustomizedComboBox_Drop(object sender, DragEventArgs e)
        {
            if (this.AllowDrop == true && e.Data.GetDataPresent(DataFormats.FileDrop))
            {
        ...
        public static readonly RoutedEvent OnFileDroppedEvent = EventManager.RegisterRoutedEvent(
            ...
            typeof(CustomizedComboBox)); // 注册事件的类

        // CLR 包装器，允许通过 += 和 -= 订阅和解除订阅事件
        [Description("Customized ComboBox Drop File Event Handler")]
        [Category("GcjControl")]
