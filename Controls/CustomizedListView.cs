...
namespace GcjUiCtrl.Control
{
    ...
    public partial class CustomizedListView : System.Windows.Controls.ListView
    {
        public CustomizedListView() : base()
        {
            //DefaultStyleKey = typeof(CustomizedListView);
            Loaded += CustomizedListView_Loaded;
            DragOver += CustomizedListView_DragOver;
            DragEnter   += CustomizedListView_DragEnter;
            Drop += CustomizedListView_Drop;
        }

        private void CustomizedListView_Loaded(object sender, RoutedEventArgs e)
        {
            Debug.WriteLine($"{string.Format("{0:d5}", ++GcjConst.nDbgSequence)}: private void CustomizedListView_Loaded(object sender, RoutedEventArgs e)\r\n Triggered(" + sender?.GetType()?.Name + " <" + (sender as FrameworkElement)?.Name + ">)");

            //Debug.WriteLine("$$$$ CustomizedListView_Loaded::(" + sender?.GetType()?.Name + ":" + (sender as FrameworkElement)?.Name + ", " + e.OriginalSource?.GetType()?.Name + ":" + (e.OriginalSource as FrameworkElement)?.Name + ") $$$$");
        ...
        public static readonly DependencyProperty _GcjVer = DependencyProperty.Register(
            "GcjVer",
            typeof(string),
            typeof(CustomizedListView),
            new PropertyMetadata("Gcj ListView v1.0"));


        [Description("Gets or sets the CustomizedListView Version")]
        [Category("GcjControl")]
        ...
        [Description("Gets or sets the CustomizedListView Drop File Path")]
        [Category("GcjControl")]
        ...
        private void CustomizedListView_DragOver(object sender, DragEventArgs e)
        {
        ...
        private void CustomizedListView_DragEnter(object sender, DragEventArgs e)
        {
            if (this.AllowDrop == true && e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                // Allow copy action
        ...
        private void CustomizedListView_Drop(object sender, DragEventArgs e)
        {
            if (this.AllowDrop == true && e.Data.GetDataPresent(DataFormats.FileDrop))
            {
        ...
        public static readonly RoutedEvent OnFileDroppedEvent = EventManager.RegisterRoutedEvent(
            ...
            typeof(CustomizedListView)); // 注册事件的类

        // CLR 包装器，允许通过 += 和 -= 订阅和解除订阅事件
        [Description("Customized ListView Control Drop File Event Handler")]
        [Category("GcjControl")]
