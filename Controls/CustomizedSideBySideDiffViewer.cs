...
namespace GcjUiCtrl.Control
{
    ...
    public partial class CustomizedSideBySideDiffViewer : GcjDiff.Wpf.Controls.SideBySideDiffViewer
    {
        public CustomizedSideBySideDiffViewer() : base()
        {
            //DefaultStyleKey = typeof(CustomizedSideBySideDiffViewer);
            Loaded += CustomizedSideBySideDiffViewer_Loaded;
            Drop += CustomizedSideBySideDiffViewer_Drop;
            DragEnter += CustomizedSideBySideDiffViewer_DragEnter;
            DragOver += CustomizedSideBySideDiffViewer_DragOver;
        }

        private void CustomizedSideBySideDiffViewer_Loaded(object sender, RoutedEventArgs e)
        {
            Debug.WriteLine($"{string.Format("{0:d5}", ++GcjConst.nDbgSequence)}: private void CustomizedSideBySideDiffViewer_Loaded(object sender, RoutedEventArgs e)\r\n Triggered(" + sender?.GetType()?.Name + " <" + (sender as FrameworkElement)?.Name + ">)");      

            //Debug.WriteLine("$$$$ CustomizedSideBySideDiffViewer_Loaded::(" + sender?.GetType()?.Name + ":" + (sender as FrameworkElement)?.Name + ", " + e.OriginalSource?.GetType()?.Name + ":" + (e.OriginalSource as FrameworkElement)?.Name + ") $$$$");
        ...
        public static readonly DependencyProperty _GcjVer = DependencyProperty.Register(
            "GcjVer",
            typeof(string),
            typeof(CustomizedSideBySideDiffViewer),
            new PropertyMetadata("Gcj SideBySideDiffViewer v1.0"));


        [Description("Gets or sets the CustomizedSideBySideDiffViewer Version")]
        [Category("GcjControl")]
        ...
        [Description("Gets or sets the CustomizedSideBySideDiffViewer Drop File Path")]
        [Category("GcjControl")]
        ...
        private void CustomizedSideBySideDiffViewer_DragOver(object sender, DragEventArgs e)
        {
        ...
        private void CustomizedSideBySideDiffViewer_DragEnter(object sender, DragEventArgs e)
        {
            if (this.AllowDrop == true && e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                // Allow copy action
        ...
        private void CustomizedSideBySideDiffViewer_Drop(object sender, DragEventArgs e)
        {
            if (this.AllowDrop == true && e.Data.GetDataPresent(DataFormats.FileDrop))
            {
        ...
        public static readonly RoutedEvent OnFileDroppedEvent = EventManager.RegisterRoutedEvent(
            ...
            typeof(CustomizedSideBySideDiffViewer)); // 注册事件的类

        // CLR 包装器，允许通过 += 和 -= 订阅和解除订阅事件
        [Description("Customized SideBySideDiffViewer Control Drop File Event Handler")]
        [Category("GcjControl")]
