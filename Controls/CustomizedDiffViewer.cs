...
namespace GcjUiCtrl.Control
{
    ...
    public partial class CustomizedDiffViewer : GcjDiff.Wpf.Controls.DiffViewer
    {
        public CustomizedDiffViewer() : base()
        {
            //DefaultStyleKey = typeof(CustomizedDiffViewer);
            Loaded += CustomizedDiffViewer_Loaded;
            Drop += CustomizedDiffViewer_Drop;
            DragEnter += CustomizedDiffViewer_DragEnter;
            DragOver += CustomizedDiffViewer_DragOver;
        }

        private void CustomizedDiffViewer_Loaded(object sender, RoutedEventArgs e)
        {
            Debug.WriteLine($"{string.Format("{0:d5}", ++GcjConst.nDbgSequence)}: private void CustomizedDiffViewer_Loaded(object sender, RoutedEventArgs e)\r\n Triggered(" + sender?.GetType()?.Name + " <" + (sender as FrameworkElement)?.Name + ">)");

            //Debug.WriteLine("$$$$ CustomizedDiffViewer_Loaded::(" + sender?.GetType()?.Name + ":" + (sender as FrameworkElement)?.Name + ", " + e.OriginalSource?.GetType()?.Name + ":" + (e.OriginalSource as FrameworkElement)?.Name + ") $$$$");
        ...
        public static readonly DependencyProperty _GcjVer = DependencyProperty.Register(
            "GcjVer",
            typeof(string),
            typeof(CustomizedDiffViewer),
            new PropertyMetadata("Gcj SideBySideDiffViewer v1.0"));


        [Description("Gets or sets the CustomizedDiffViewer Version")]
        [Category("GcjControl")]
        ...
        [Description("Gets or sets the CustomizedDiffViewer Drop File Path")]
        [Category("GcjControl")]
        ...
        private void CustomizedDiffViewer_DragOver(object sender, DragEventArgs e)
        {
        ...
        private void CustomizedDiffViewer_DragEnter(object sender, DragEventArgs e)
        {
            if (this.AllowDrop == true && e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                // Allow copy action
        ...
        private void CustomizedDiffViewer_Drop(object sender, DragEventArgs e)
        {
            if (this.AllowDrop == true && e.Data.GetDataPresent(DataFormats.FileDrop))
            {
        ...
        public static readonly RoutedEvent OnFileDroppedEvent = EventManager.RegisterRoutedEvent(
            ...
            typeof(CustomizedDiffViewer)); // 注册事件的类
