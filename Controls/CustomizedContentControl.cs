...
namespace GcjUiCtrl.Control
{
    ...
    public partial class CustomizedContentControl : System.Windows.Controls.ContentControl
    {
        public CustomizedContentControl() : base()
        {
            //DefaultStyleKey = typeof(CustomizedContentControl);
            Loaded += CustomizedContentControl_Loaded;
            Drop += CustomizedContentControl_Drop;
            DragEnter += CustomizedContentControl_DragEnter;
            DragOver += CustomizedContentControl_DragOver;
        }

        private void CustomizedContentControl_Loaded(object sender, RoutedEventArgs e)
        {
            Debug.WriteLine($"{string.Format("{0:d5}", ++GcjConst.nDbgSequence)}: private void CustomizedContentControl_Loaded Triggered(" + sender?.GetType()?.Name + " <" + (sender as FrameworkElement)?.Name + ">)");
            //Debug.WriteLine("$$$$ CustomizedContentControl_Loaded::(" + sender?.GetType()?.Name + ":" + (sender as FrameworkElement)?.Name + ", " + e.OriginalSource?.GetType()?.Name + ":" + (e.OriginalSource as FrameworkElement)?.Name + ") $$$$");
        ...
        public static readonly DependencyProperty _GcjVer = DependencyProperty.Register(
            "GcjVer",
            typeof(string),
            typeof(CustomizedContentControl),
            new PropertyMetadata("Gcj ContentControl v1.0"));


        [Description("Gets or sets the CustomizedContentControl Version")]
        [Category("GcjControl")]
        ...
        [Description("Gets or sets the CustomizedContentControl Drop File Path")]
        [Category("GcjControl")]
        ...
        private void CustomizedContentControl_DragOver(object sender, DragEventArgs e)
        {
        ...
        private void CustomizedContentControl_DragEnter(object sender, DragEventArgs e)
        {
            if (this.AllowDrop == true && e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                // Allow copy action
        ...
        private void CustomizedContentControl_Drop(object sender, DragEventArgs e)
        {
            if (this.AllowDrop == true && e.Data.GetDataPresent(DataFormats.FileDrop))
            {
        ...
        public static readonly RoutedEvent OnFileDroppedEvent = EventManager.RegisterRoutedEvent(
            ...
            typeof(CustomizedContentControl)); // 注册事件的类

        // CLR 包装器，允许通过 += 和 -= 订阅和解除订阅事件
        [Description("Customized Content Control Drop File Event Handler")]
        [Category("GcjControl")]
