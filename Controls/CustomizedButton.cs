...
namespace GcjUiCtrl.Control
{
    ...
    public partial class CustomizedButton : System.Windows.Controls.Button
    {
        public CustomizedButton() : base()
        {
            //DefaultStyleKey = typeof(CustomizedButton);
            Loaded += CustomizedButton_Loaded;
            Drop += CustomizedButton_Drop1;
            DragEnter += CustomizedButton_DragEnter;  
            DragOver += CustomizedButton_DragOver;
        }

        private void CustomizedButton_Drop1(object sender, DragEventArgs e)
        {
            throw new NotImplementedException();
        }

        private void CustomizedButton_Loaded(object sender, RoutedEventArgs e)
        {
            Debug.WriteLine($"{string.Format("{0:d5}", ++GcjConst.nDbgSequence)}: private void CustomizedButton_Loaded Triggered(" + sender?.GetType()?.Name + " <" + (sender as FrameworkElement)?.Name + ">)");
            //Debug.WriteLine("$$$$ CustomizedButton_Loaded::(" + sender?.GetType()?.Name + ":" + (sender as FrameworkElement)?.Name + ", " + e.OriginalSource?.GetType()?.Name + ":" + (e.OriginalSource as FrameworkElement)?.Name + ") $$$$");
        ...
        public static readonly DependencyProperty _GcjVer = DependencyProperty.Register(
            "GcjVer",
            typeof(string),
            typeof(CustomizedButton),
            new PropertyMetadata("Gcj Button v1.0"));


        [Description("Gets or sets the CustomizedButton Version")]
        [Category("GcjControl")]
        ...
        [Description("Gets or sets the CustomizedButton Drop File Path")]
        [Category("GcjControl")]
        ...
        private void CustomizedButton_DragOver(object sender, DragEventArgs e)
        {
        ...
        private void CustomizedButton_DragEnter(object sender, DragEventArgs e)
        {
            if (this.AllowDrop == true && e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                // Allow copy action
        ...
        private void CustomizedButton_Drop(object sender, DragEventArgs e)
        {
            if (this.AllowDrop == true && e.Data.GetDataPresent(DataFormats.FileDrop))
            {
